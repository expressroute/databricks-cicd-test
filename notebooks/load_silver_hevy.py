...
print(f"stg_df count: {stg_df.count()}")
...
from pyspark.sql.functions import sha2, concat_ws, col, to_timestamp, when, current_timestamp
...
cleaned_df = (
    ...
    .select(
        ...
        when(col("exercise_title") == "Running", True).otherwise(False).alias("is_running"),
        when(col("exercise_title") != "Running", True).otherwise(False).alias("is_workout"),
    )
...
# MAGIC %sql
# MAGIC
# MAGIC select * from hen_db.stg.silver_workout
